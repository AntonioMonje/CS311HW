...
//Complier:  g++
//File type: Implemenatation file
//================================================================
...
void BST::DeleteVertex(elem_t E)
{
  ...
  if(((E == Root->Elem) && (Root->Left != NULL) && (Root->Right == NULL)) || ((E == Root->Elem) && (Root->Left == NULL) && (Root->Right != NULL)))
    { 
// ** change the root to the child and return - this could
      //be the left or the right child 
      cout << "...deleteing the root" << endl;
      if(Root->Left != NULL)
	{
	  V = Root -> Left;
	  delete Root;
...
int BST::greaterof(Vertex *LH, Vertex *RH)
{
  int L;
  int R;  
  if(Root == NULL)
    ...
    { 
      L = greaterof(LH->Left, RH->Left);
      R = greaterof(LH->Right, RH->Right);
      if(L > R)
	{
	  return (L + 1);
	}
...
/*
int BST::diff(Vertex* V, Vertex* D)
{
  int Height;
  int Balance;
  int Compute;
